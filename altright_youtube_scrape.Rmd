...
milo_ids  <- get_video_ids("text/milo.txt")
...
milo_ids %<>% 
 filter(date > as_date("2017-01-01") & date < as_date("2018-01-01")) %>% 
  ...
  as.vector()
## Try 1
milo_comments <- extract_comments(milo_ids$yt_code, 1, "milo")


pjw_ids  <- get_video_ids("text/pjw.txt")
```
...
types <- c("cernovich", "lauren", "pjw", "milo", "roaming")
