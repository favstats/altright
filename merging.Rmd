...
fb_all %>% 
  ...
  filter(stringr::str_detect(message, "establishment")) #%>% 
  # group_by(page) %>% 
  # summarise(n = n())  %>%
  # left_join(fb_all %>% 
  # group_by(page) %>% 
...
```{r}
tw_sets <- dir(here::here("data"))[-c(1:2, 58:60)]

dat_list <- list()
for (jj in seq_along(tw_sets)) {
  dat_list[[jj]] <- do.call("rbind",(get(load(here("data", tw_sets[jj])))))
}
tw_partial2 <- do.call("rbind", dat_list)
save(tw_partial2, file = here("data", "tw", "tw_partial2.Rdata"))

table(tw_partial2$handle)
...
tw_partial1_2 <- rbind(tw_partial1, tw_partial2)

save(tw_partial1_2, file = here("data", "tw", "tw_partial1_2.Rdata"))

table(tw_partial1_2$handle)
