...
anncoulter_tweets <- rtweet::lookup_tweets(an_ids)
#save(anncoulter_tweets, file = "data/anncoulter_tweets.Rdata")
glimpse(anncoulter_tweets)
...
handle <- "anncoulter"
ids <- an_ids
test <- get_replies_df("anncoulter", ids[1:10])
...
```{r}
dat <- data.frame(handler = rep("anncoulter", 2), tweet_id =  ids[1:2])
