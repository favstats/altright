...
```{r}
pacman::p_load(tidyverse, dplyr, ggplot2, stringr, scales, lubridate, tidytext, tidyr, ggthemes, flexdashboard, Rfacebook, here, magrittr, gdata, rlist, rjson)
```
...
## download posts from page

helper function to extract comments

```{r}
get_comments <- function(post_ids){
  data_list <- list()
  
  for (jj in seq_along(post_ids)) {
    
    tryCatch({
      dat <- Rfacebook::getPost(
        post = post_ids[jj], 
        token = fb_oauth,
        n = 20000,
        #likes = T, 
...
apply get_comments function in get_facebook

```{r}
get_facebok <- function(page_ids){
  
  data_list <- list()
  
  for(jj in seq_along(page_ids)) {
    ...
    comments <- get_comments(post_ids = page$id)
...
# application
page_ids <- id_data$FacebookID
get_facebok(page_ids[1:2])
```
